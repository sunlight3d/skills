---
name: pptx-add-cecomtech-logo
description: >-
  Adds the Cecomtech logo to the bottom right corner of Google Slides presentations, covering previous watermark logos. Supports both individual presentations and Google Drive folders.
---

# PPTX Add Cecomtech Logo

## Overview
This skill automatically covers any previous logos (like NotebookLM watermarks) at the bottom right corner of a Google Slides presentation by first inserting a white rectangle with no border, and then overlaying the Cecomtech logo. It uses the Google Slides API to batch update all slides in a presentation, and intelligently skips slides that already contain the logo.
...
## Configuration
The script calculates the size of the logo maintaining aspect ratio (3786x1120) with a fixed width of `2,000,000 EMU` (~157 pt). It leaves a small margin of `100,000 EMU` from the bottom-right edges to perfectly cover most typical corner watermarks.
...
uv run pptx_add_cecomtech_logo.py <PRESENTATION_ID> --credentials /path/to/credentials.json

# Process all presentations inside a Google Drive folder
uv run pptx_add_cecomtech_logo.py <FOLDER_ID> --folder --credentials /path/to/credentials.json
```
