...
echo "Starting git pull across specified directories..."
...
for DIR in "${DIRS[@]}"; do
    ...
    while IFS= read -r gitdir; do
        ...
        # Convert HTTPS GitHub remote URL to SSH to allow passwordless pull
        ...
        if [ -n "$(git status --porcelain)" ]; then
            echo "⚠️  Uncommitted changes/untracked files detected in $repo_dir. Auto-committing to avoid pull conflicts..."
            git add -A
